...
def draw_dungeon(bg, fnt):  # ダンジョンを描画する
    ...
    bg.blit(imgDark, [0, 0])  # ４隅が暗闇の画像を重ねる
...
def init_battle_mimic():                    # 宝箱を取ると稀にミミックと戦闘が起こる    オリジナル
    global imgEnemy, emy_name, emy_lifemax, emy_life, emy_str, emy_x, emy_y
    treasure == 5
    typ = 10
    lev = random.randint(1, floor)
...
def main(): # メイン処理
    ...
    font = pygame.font.Font("IPAexfont/ipaexg.ttf", 30)             # 日本語表記に変更  オリジナル
    fontS = pygame.font.Font("IPAexfont/ipaexg.ttf", 20)            # 日本語表記に変更  オリジナル
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key == K_q:    # qでセーブ
                    file = open("save.txt", 'w')    # save.txtを作成・上書きさせる
                ...
                if event.key == K_z:        # zでロード
                    file = open("save.txt", 'r')    # save.txtを読み込む
        ...
        if idx == 0:  # タイトル画面
            ...
            draw_text(screen, "スペースキーでスタート", 280, 560, font, BLINK[tmr%6])
            if os.path.isfile('save.txt'):                                                # フォルダにsave.txtがあれば
                draw_text(screen, "Zキーで途中からスタート", 280, 610, font, BLINK[tmr%6])    # タイトル画面に表示させる
            if key[K_SPACE] == 1:
                make_dungeon()
                put_event()
                floor = 1
                ...
                pygame.mixer.music.play(-1)
            if key[K_z] == 1:       # zキーで保存された情報からスタート
                if event.key == K_z:
                    make_dungeon()
                    put_event()
                    welcome = 15
                    os.remove('save.txt')   # ロードした際にsave.txtを削除する
                    idx = 1
                    pygame.mixer.music.load("sound/ohd_bgm_field.ogg")
                    pygame.mixer.music.play(-1)
            ...
            if tmr == 1:
                ...
                draw_text(screen, "モンスターと遭遇!", 350, 200, font, WHITE)
            elif tmr <= 16:
                draw_battle(screen, fontS)
                draw_text(screen, emy_name+" が出現した!", 300, 200, font, WHITE)
            else:
                idx = 11
                tmr = 0
        
        elif idx == 18:    # ミミックとの戦闘開始   オリジナル
            if tmr == 1:
                pygame.mixer.music.load("sound/ohd_bgm_battle.ogg")
                pygame.mixer.music.play(-1)
                init_battle_mimic()
                ...
                draw_battle(screen, fontS)
                draw_text(screen, "宝箱は "+emy_name+" だった!", 300, 200, font, WHITE)
            else:
                idx = 11
                tmr = 0
            ...
            if tmr == 9:
                ...
                set_message(str(dmg)+"のダメージをくらった")
                dmg_eff = 5
                emy_step = 0
            if tmr == 15:
                pl_life = pl_life - dmg
        ...
        draw_text(screen, "[Q]save ", 740, 80, fontS, WHITE)
